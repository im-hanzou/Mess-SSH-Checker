...
def console_progress(value, total):
    ...
    progress_indicator = '#' * value_width + '-' * (PROGRESS_WIDTH - value_width)
...
def worker(in_q, result_q, timeout):
    while not in_q.empty():
        try:
            row = in_q.get()
            host, username, password = row.split('|')
            # 5 - неверные логин и/или пароль
            # 255 - порт закрыт
            retcode = check_ssh(username, password, host, timeout)
            if retcode == 0:
                result_q.put_nowait(
                    {
                        'type': 'success',
...
def main():
    ...
    parser.add_argument(
        ...
        '--input',
        default=sys.stdin,
        help='SSH credentials in CSV format. Fields: host|user|pass. Default: <STDIN>',
